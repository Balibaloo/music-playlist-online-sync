use music_file_playlist_online_sync::api::{
    mock::MockProvider, spotify::SpotifyProvider, tidal::TidalProvider, Provider,
};
...
async fn test_provider_auth_playlist_ops() {
    ...
    if !tidal_id.is_empty() && !tidal_secret.is_empty() {
        ...
        let pl_id = tidal
            .ensure_playlist(&new_name, "Auth test playlist")
            .await
            .expect("Tidal create playlist");
        println!("Created Tidal playlist: {} ({})", new_name, pl_id);
        let rename_name = format!("Test MFPOS {}", new_n + 1);
        tidal
            .rename_playlist(&pl_id, &rename_name)
            .await
            .expect("Tidal rename playlist");
